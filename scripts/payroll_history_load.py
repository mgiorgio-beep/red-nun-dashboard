...
Input: a folder holding the 7shifts payroll-journal CSVs in chatham/ and
dennis/ subfolders (the entity is the folder; the CSV does not say).
...
EXPECTED = {("dennis", d) for d in ("2026-01-09", "2026-01-23", "2026-02-06", "2026-02-20", "2026-03-06",
                                    "2026-03-20", "2026-04-03", "2026-05-15")} | {("chatham", "2026-04-03")}
...
def load(conn, loc, path, now, apply):
    emps = parse_journal_csv(open(path, encoding="utf-8-sig").read())
    ...
    existing = conn.execute("SELECT * FROM payroll_runs WHERE location=? AND pay_period_start=? AND pay_period_end=? "
                            "AND total_gross > 1000", (loc, start, end)).fetchall()
    assert len(existing) <= 1, f"{loc} {start}..{end}: {len(existing)} runs on file"
    msg = f"{loc:<8} pay {pay}  period {start}..{end}  {t['employee_count']:>3} employees  gross {t['total_gross']:>10,.2f}"
    if existing:
        run = existing[0]
        ...
        run_id = conn.execute(
            ...
            (loc, start, end, pay, f"Payroll {start}..{end} — {WHO}: historical 7shifts journal", dest,
             *t.values())).lastrowid
        for e in emps:
...
def main():
    ...
    try:
        ...
        for loc in ("chatham", "dennis"):
            for f in sorted(glob.glob(os.path.join(a.dir, loc, "*.csv"))):
                msg, key = load(conn, loc, f, now, a.apply)
